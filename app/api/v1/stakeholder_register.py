...
from app.schemas.stakeholder_register import (
    StakeholderGenerateResponse,
...
def format_stakeholder_response(ai_response_data):
    if isinstance(ai_response_data, dict):
        return ai_response_data.get("content", "")
    return str(ai_response_data)


@router.post("/generate", response_model=StakeholderGenerateResponse)
async def generate_stakeholder_register(
    project_id: int = Form(...),
    ...
    logger.info(
        f"User {current_user.email} requested Stakeholder Register generation for {project_name}"
    )
    ...
    unique_title = get_unique_diagram_name(
        db, project_name, project_id, "stakeholder_register"
    )
    ...
    # Gọi AI service
    ai_data = await call_ai_service(settings.ai_service_url_stakeholder, ai_payload)

    markdown_content = format_stakeholder_response(ai_data["response"])
    ...
    try:
        new_file = Files(
            ...
            file_type="stakeholder_register",
            metadata={
                "message": description,
                "ai_response": ai_data,
            },
        ...
        new_ai_session = Chat_Session(
            ...
            content_type="stakeholder_register",
        ...
        return StakeholderGenerateResponse(
        ...
        db.rollback()  
        logger.error(f"Database error during stakeholder generation: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")


@router.get("/list/{project_id}", response_model=StakeholderListResponse)
async def list_stakeholder_register(
    project_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
...
@router.get("/get/{project_id}/{document_id}", response_model=GetStakeholderResponse)
async def get_stakeholder_document(
    project_id: str,
    document_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
...
@router.put(
    "/update/{project_id}/{document_id}", response_model=UpdateStakeholderResponse
)
async def update_stakeholder_register(
    project_id: str,
    document_id: str,
    content: str = Form(...),
    ...
    logger.info(
        f"User {current_user.email} requested update for stakeholder doc {document_id}"
    )
    file_type = "stakeholder_register"
    doc = (
        db.query(Files)
    ...
    return UpdateStakeholderResponse(
        document_id=str(doc.id),
        project_name=doc.name,
        content=content,
...
@router.patch(
    "/regenerate/{project_id}/{document_id}", response_model=StakeholderGenerateResponse
)
async def regenerate_stakeholder_register(
    project_id: int,
    document_id: str,
    description: Optional[str] = Form(""),
    db: Session = Depends(get_db),
    ...
    logger.info(
        f"User {current_user.email} requested Stakeholder regeneration for {document_id}"
    )
    file_type = "stakeholder_register"
    existing_doc = (
        db.query(Files)
    ...
    # file_urls = await list_file(project_id, db, current_user)
    # ai_payload = {"message": description,  "content_id": document_id,"storage_paths": file_urls}
