...
async def upload_to_supabase(file: UploadFile) -> str | None:
    ...
    try:
        ...
        path_in_bucket = public_url[index + len(SUPABASE_BUCKET) + 2 :]
    ...
    for file in existing_files_db:
        try: 
            ...
            existing_files_uploadfile.append(
                UploadFile(filename=file.file_path.split("/")[-1], file=io.BytesIO(file_bytes))
            )
        except Exception as e:
            logger.exception(
                f"Error when getting file '{file}': {e}"
            )
